...
pub fn node_unsafe_block_parser<'src>() -> impl Parser<'src, &'src [Token], Node, extra::Err<Rich<'src, Token>>> {
    just(Token::OpenUnsafe)
        ...
        .map_with(|(reason, children), extra| {
            Node::Unsafe(UnsafeNode { reason, children, span: to_span(extra.span()) })
        })
}

pub fn node_else_if_block_parser<'src>(
    node: impl Parser<'src, &'src [Token], Node, extra::Err<Rich<'src, Token>>> + Clone,
) -> impl Parser<'src, &'src [Token], IfNode, extra::Err<Rich<'src, Token>>> {
    just(Token::OpenElseIf)
        ...
        .map_with(|(condition, then_children), extra| IfNode {
            condition,
            then_children,
...
pub fn node_if_block_parser<'src>(
    node: impl Parser<'src, &'src [Token], Node, extra::Err<Rich<'src, Token>>> + Clone,
    ...
    just(Token::OpenIf)
        ...
        .then_ignore(just(Token::EndTag))
        .then(node.repeated().collect::<Vec<Node>>())
        // closes </if> — elseIf and else follow as siblings outside this tag
        .then_ignore(select! { Token::CloseTag(_) => () })
        .then(else_if.repeated().collect::<Vec<IfNode>>())
        .then(else_branch.or_not())
        .map_with(|(((condition, then_children), else_if_children), else_child), extra| {
            Node::If(IfNode {
                condition,
                then_children,
                else_if_children,
...
pub fn node_each_block_parser<'src>(
    ...
    just(Token::OpenEach)
        ...
        .then(node.repeated().collect::<Vec<Node>>())
        .then_ignore(select! { Token::CloseTag(_) => () })
        .map_with(|(((collection, alias), index_alias), children), extra| {
            Node::Each(EachNode {
                collection,
                alias,
                index_alias,
...
pub fn node_parser<'src>() -> impl Parser<'src, &'src [Token], Node, extra::Err<Rich<'src, Token>>> {
    recursive::<_, _, extra::Err<Rich<'src, Token>>, _, _>(|node| {
        ...
        let open_close = just(Token::StartTag)
            ...
            .then(node.repeated().collect::<Vec<Node>>())
            .then_ignore(select! { Token::CloseTag(_) => () })
            .map_with(|((name, attrs), children), extra| {
                Node::Component(ComponentNode {
                    name,
                    attrs,
                    children,
...
fn body_parser<'src>() -> impl Parser<'src, &'src [Token], Body, extra::Err<Rich<'src, Token>>> {
    just(Token::OpenBody)
        ...
        .then(node_parser().repeated().collect::<Vec<Node>>())
        .map(|(block, children)| Body {
            logic_block: block.unwrap_or_default(),
            template: children,
        })
...
fn layout_def_parser<'src>() -> impl Parser<'src, &'src [Token], Declaration, extra::Err<Rich<'src, Token>>> {
    just(Token::KwLayout)
        .ignore_then(select! { Token::Ident(name) => name })
        .then(body_parser())
        .map(|(name, body)| Declaration::Layout { name, body })
}
...
fn page_def_parser<'src>() -> impl Parser<'src, &'src [Token], Declaration, extra::Err<Rich<'src, Token>>> {
    just(Token::KwPage)
        ...
        .then(props_parser())
        .then(body_parser())
        .map(|((name, props), body)| Declaration::Page { name, props, body })
}
...
fn component_def_parser<'src>() -> impl Parser<'src, &'src [Token], Declaration, extra::Err<Rich<'src, Token>>> {
    just(Token::KwComponent)
        ...
        .then(props_parser())
        .then(body_parser())
        .map(|((name, props), body)| Declaration::Component { name, props, body })
}
